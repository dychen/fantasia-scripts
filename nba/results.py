...
def run_weighted():
    salaries = load_salaries('salaries/dk_nba_salaries_2015_11_12.csv')
    ...
    for i, contest_id in enumerate(ids):
        ...
        scores_weighted = analyze_results(
            results, lambda x: sum([get_weighted_score(y) for y in x]) / float(len(x))
        )
    ...
    for player, scores in scores_weighted_all.iteritems():
        scores_weighted_all[player] = (numpy.mean([math.log(abs(s)) * numpy.sign(s) for s in scores]), len(scores))
    sorted_scores = sorted(scores_weighted_all.items(), key=lambda x: x[1], reverse=True)
    for player, score in sorted_scores:
        if player in salaries:
            print '\t%s\t%.4f\t%d\t%s' % (player[:15], score[0], score[1], salaries[player])
        else:
...
def run():
    ...
    scores_mean = analyze_results(results, lambda x: numpy.mean(x) / len(results))
    scores_own = analyze_results(results, lambda x: len(x) / float(len(results)))
    scores_weighted = analyze_results(
        results, lambda x: sum([get_weighted_score(y) for y in x]) / float(len(x))
    )
    sorted_mean = sorted(scores_mean.items(), key=lambda x: x[1], reverse=False)
    sorted_own = sorted(scores_own.items(), key=lambda x: x[1], reverse=True)
    sorted_weighted = sorted(scores_weighted.items(), key=lambda x: x[1], reverse=True)
    ...
    for player, weighted_score in sorted_weighted[:50]:
        if player in salaries:
            print ('\t%s\t%.4f\t%.2f\t%.4f\t%s'
                   % (player[:15], scores_mean[player], scores_own[player] * 100,
                      weighted_score, salaries[player]))
        else:
            print ('\t%s\t%.4f\t%.2f\t%.4f'
                   % (player[:15], scores_mean[player], scores_own[player] * 100,
                      weighted_score))
